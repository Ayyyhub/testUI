                ...
                try:
                    ...
                    element = wait.until(
                        EC.element_to_be_clickable(
                            (
                                By.XPATH,
                                '//li[contains(@class,"el-menu-item")][5]/div//span[text()="上传"]',
                            )
                        )
                    )
                    element.click()
                    ...
                    try:
                        ...
                        for selector in file_input_selectors:
                            try:
                                ...
                                logger.warning(
                                    f"尝试选择器 {selector} 失败: {str(e)}"
                                )

                        if not file_input:
                            # 如果没有找到，尝试使用JavaScript查找所有input[type=file]
                            logger.info(
                                "通过XPath未找到文件输入框，尝试使用JavaScript..."
                            )
                        ...
                        if file_input:
                            ...
                            file_path = os.path.abspath(
                                r"D:\AE_myspace\Ae_机器人场景\BXMD74.step"
                            )
                            ...
                            try:
                                ...
                                while (
                                    ...
                                    if upload_complete:
                                        logger.info(
                                            f"文件上传成功，耗时: {time.time() - start_time:.2f}秒"
                                        )
                                        upload_success = True
                                        break
                                    ...
                                    if int(
                                        ...
                                        logger.info(
                                            f"正在匹配元素,等待上传完成，已等待: {time.time() - start_time:.2f}秒"
                                        )
                                ...
                                if not upload_complete:
                                    logger.warning(
                                        f"上传等待超时，已等待{max_wait_time}秒，继续执行后续步骤"
                                    )
